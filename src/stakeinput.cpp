...
bool LambyCStake::InitFromTxIn(const CTxIn& txin)
{
...
bool LambyCStake::SetPrevout(CTransaction txPrev, unsigned int n)
{
    this->txFrom = txPrev;
    this->nPosition = n;
    return true;
...
bool LambyCStake::GetTxFrom(CTransaction& tx) const
{
    if (txFrom.IsNull())
        return false;
    tx = txFrom;
...
bool LambyCStake::GetTxOutFrom(CTxOut& out) const
{
...
bool LambyCStake::CreateTxIn(CWallet* pwallet, CTxIn& txIn, uint256 hashTxOut)
{
    txIn = CTxIn(txFrom.GetHash(), nPosition);
    return true;
}

CAmount LambyCStake::GetValue() const
{
    return txFrom.vout[nPosition].nValue;
}

bool LambyCStake::CreateTxOuts(CWallet* pwallet, std::vector<CTxOut>& vout, CAmount nTotal)
{
...
CDataStream LambyCStake::GetUniqueness() const
{
    //The unique identifier for a Lamby stake is the outpoint
...
CBlockIndex* LambyCStake::GetIndexFrom()
{
    if (pindexFrom)
        return pindexFrom;
...
bool LambyCStake::ContextCheck(int nHeight, uint32_t nTime)
{
