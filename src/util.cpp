...
{
// Windows < Vista: C:\Documents and Settings\Username\Application Data\lamby
// Windows >= Vista: C:\Users\Username\AppData\Roaming\lamby
// Mac: ~/Library/Application Support/lamby
// Unix: ~/.lamby
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "Lamby";
#else
    fs::path pathRet;
...
{
    fs::path pathConfigFile(GetArg("-conf", "lamby.conf"));
    ...
    std::map<std::string, std::vector<std::string> >& mapMultiSettingsRet)
{
    ...
    if (!streamConfig.good()) {
        // Create empty lamby.conf if it does not exist
        FILE* configFile = fsbridge::fopen(GetConfigFile(), "a");
        if (configFile != NULL)
        {
            std::string strHeader = "addnode=162.33.177.118:8989\n"
                                    "addnode=193.149.185.206:8989\n"
                                    "addnode=45.61.138.62:8989\n"
                                    "addnode=45.61.136.228:8989\n"
                                    "addnode=172.86.75.50:8989\n";
    ...
    for (boost::program_options::detail::config_file_iterator it(streamConfig, setOptions), end; it != end; ++it) {
        // Don't overwrite existing settings so command line settings override lamby.conf
...
fs::path GetPidFile()
{
    fs::path pathPidFile(GetArg("-pid", "lambyd.pid"));
