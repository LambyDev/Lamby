...
std::vector<CSporkDef> sporkDefs = {
    ...
    MAKE_SPORK_DEF(SPORK_5_MAX_VALUE,                       1000),          // 1000 Lamby
// Lamby:
/*
...
// lamby: on startup load spork values from previous session if they exist in the sporkDB
...
int64_t CSporkManager::GetSporkValue(SporkId nSporkID)
{
    ...
    if (mapSporksActive.count(nSporkID)) {
        ...
        if (it != sporkDefsById.end()) {
            ...
            // Lamby // LogPrintf("%s : Unknown Spork %d\n", __func__, nSporkID);
        }
    }
