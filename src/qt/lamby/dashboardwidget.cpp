...
#include "qt/lamby/dashboardwidget.h"
#include "qt/lamby/forms/ui_dashboardwidget.h"
#include "qt/lamby/sendconfirmdialog.h"
#include "qt/lamby/txrow.h"
...
DashboardWidget::DashboardWidget(lambyGUI* parent) :
    PWidget(parent),
    ui(new Ui::DashboardWidget)
{
    ...
    setCssProperty(ui->labelSquareLamby, "square-chart-lamby");
    setCssProperty(ui->labelLamby, "text-chart-lamby");
    ...
    setCssProperty(ui->labelAmountLamby, "text-stake-lamby-disable");
...
bool hasCharts = false;
    ...
    if (window)
        connect(window, &lambyGUI::windowResizeEvent, this, &DashboardWidget::windowResizeEvent);
#endif
...
void DashboardWidget::loadWalletModel()
{
    if (walletModel && walletModel->getOptionsModel()) {
        ...
        connect(ui->pushImgEmpty, &QPushButton::clicked, window, &lambyGUI::openFAQ);
        connect(ui->btnHowTo, &QPushButton::clicked, window, &lambyGUI::openFAQ);
...
bool DashboardWidget::loadChartData(bool withMonthNames)
{
    ...
    for (int j = range.first; j < range.second; j++) {
        ...
        if (chartData->amountsByCache.contains(num)) {
            ...
            lamby = (pair.first != 0) ? pair.first / 100000000 : 0;
            chartData->totalLamby += pair.first;
        }

        chartData->xLabels << ((withMonthNames) ? monthsNames[num - 1] : QString::number(num));

        chartData->valuesLamby.append(lamby);

        int max = std::max(lamby, lamby);
...
void DashboardWidget::onChartRefreshed()
{
    ...
    set0->append(chartData->valuesLamby);

    // Total
    nDisplayUnit = walletModel->getOptionsModel()->getDisplayUnit();
    if (chartData->totalLamby > 0) {
        setCssProperty(ui->labelAmountLamby, "text-stake-lamby");
    } else {
        setCssProperty(ui->labelAmountLamby, "text-stake-lamby-disable");
    }
    forceUpdateStyle({ui->labelAmountLamby});
    ui->labelAmountLamby->setText(GUIUtil::formatBalance(chartData->totalLamby, nDisplayUnit));
