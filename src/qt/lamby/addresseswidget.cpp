...
#include "qt/lamby/forms/ui_addresseswidget.h"
#include "qt/lamby/addresslabelrow.h"
#include "qt/lamby/addnewaddressdialog.h"
#include "qt/lamby/tooltipmenu.h"
...
AddressesWidget::AddressesWidget(lambyGUI* parent) :
    PWidget(parent),
...
void AddressesWidget::onStoreContactClicked()
{
    if (walletModel) {
        ...
        CTxDestination lambyAdd = DecodeDestination(address.toUtf8().constData(), isStakingAddress);
        ...
        if (walletModel->updateAddressBookLabels(lambyAdd, label.toUtf8().constData(),
