...
#include "qt/lamby/forms/ui_send.h"
#include "qt/lamby/addnewcontactdialog.h"
#include "qt/lamby/qtutils.h"
#include "qt/lamby/sendchangeaddressdialog.h"
#include "qt/lamby/optionbutton.h"
#include "qt/lamby/sendconfirmdialog.h"
#include "qt/lamby/myaddressrow.h"
#include "qt/lamby/guitransactionsutils.h"
...
SendWidget::SendWidget(lambyGUI* parent) :
    PWidget(parent),
    ui(new Ui::send),
    ...
    btnContacts(new QPushButton())
{
    ...
    setCssProperty(coinIcon, "coin-icon-lamby");
...
void SendWidget::onContactMultiClicked()
{
    if (focusedEntry) {
        ...
        CTxDestination lambyAdd = DecodeDestination(address.toStdString());
        if (walletModel->isMine(lambyAdd)) {
        ...
        if (dialog->res) {
            ...
            if (walletModel->updateAddressBookLabels(lambyAdd, dialog->getLabel().toStdString(),
                    AddressBook::AddressBookPurpose::SEND)) {
