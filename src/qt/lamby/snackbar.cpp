...
#include "qt/lamby/forms/ui_snackbar.h"
#include "qt/lamby/qtutils.h"
#include <QTimer>


SnackBar::SnackBar(lambyGUI* _window, QWidget *parent) :
    QDialog(parent),
    ui(new Ui::SnackBar),
    window(_window),
    timeout(MIN_TIMEOUT)
{
    ...
    if (window)
        connect(window, &lambyGUI::windowResizeEvent, this, &SnackBar::windowResizeEvent);
    else {
