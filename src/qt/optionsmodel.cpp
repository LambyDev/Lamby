...
void OptionsModel::setDisplayDefaultOptions(QSettings& settings, bool reset)
{
    if (!settings.contains("nDisplayUnit") || reset)
        settings.setValue("nDisplayUnit", BitcoinUnits::Lamby);
    ...
    if (settings.contains("nAnonymizelambyAmount") || reset)
        SoftSetArg("-anonymizelambyamount", settings.value("nAnonymizelambyAmount").toString().toStdString());
...
void OptionsModel::Reset()
{
    ...
    settings.clear();
    resetSettings = true; // Needed in lamby.cpp during shotdown to also remove the window positions
