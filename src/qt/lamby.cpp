...
#include "qt/lamby/splash.h"
#include "qt/lamby/welcomecontentwidget.h"
...
{
    return QCoreApplication::translate("lamby-core", psz).toStdString();
}
...
static void initTranslations(QTranslator& qtTranslatorBase, QTranslator& qtTranslator, QTranslator& translatorBase, QTranslator& translator, bool forceLangFromSettings = false)
{
    ...
    // Load e.g. bitcoin_de.qm (shortcut "de" needs to be defined in lamby.qrc)
    if (translatorBase.load(lang, ":/translations/"))
        QApplication::installTranslator(&translatorBase);

    // Load e.g. bitcoin_de_DE.qm (shortcut "de_DE" needs to be defined in lamby.qrc)
...
/** Class encapsulating lamby startup and shutdown.
...
public:
    ...
    /// Get window identifier of QMainWindow (lambyGUI)
...
{
    window = new lambyGUI(networkStyle, 0);

    pollShutdownTimer = new QTimer(window);
    connect(pollShutdownTimer, &QTimer::timeout, window, &lambyGUI::detectShutdown);
...
void BitcoinApplication::startThread()
{
    ...
    connect(window, &lambyGUI::requestedRestart, executor, &BitcoinCore::restart);
...
void BitcoinApplication::initializeResult(int retval)
{
    ...
    if (retval) {
        ...
        if (pwalletMain) {
            ...
            window->addWallet(lambyGUI::DEFAULT_WALLET, walletModel);
            window->setCurrentWallet(lambyGUI::DEFAULT_WALLET);
        ...
        // lamby: URIs or payment requests:
        //connect(paymentServer, &PaymentServer::receivedPaymentRequest, window, &lambyGUI::handlePaymentRequest);
        connect(window, &lambyGUI::receivedURI, paymentServer, &PaymentServer::handleURIOrFile);
...
{
    QMessageBox::critical(0, "Runaway exception", QObject::tr("A fatal error occurred. Lamby can no longer continue safely and will quit.") + QString("\n\n") + message);
    ::exit(1);
}
...
/// 2. Basic Qt initialization (not dependent on parameters or configuration)
    Q_INIT_RESOURCE(lamby_locale);
    Q_INIT_RESOURCE(lamby);
    ...
    /// 6. Determine availability of data directory and parse lamby.conf
    /// - Do not call GetDataDir(true) before this step finishes
    if (!fs::is_directory(GetDataDir(false))) {
        QMessageBox::critical(0, QObject::tr("Lamby"),
    ...
    if (!SelectParamsFromCommandLine()) {
        QMessageBox::critical(0, QObject::tr("Lamby"), QObject::tr("Error: Invalid combination of -regtest and -testnet."));
        return 1;
    }
#ifdef ENABLE_WALLET
    ...
    // lamby: links repeatedly have their payment requests routed to this process:
...
#if defined(Q_OS_WIN)
        WinShutdownMonitor::registerShutdownBlockReason(QObject::tr("Lamby didn't yet exit safely..."), (HWND)app.getMainWinId());
#endif
        app.exec();
        app.requestShutdown();
        app.exec();
