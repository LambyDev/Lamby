...
The build script we'll be using is contained in the lamby github repository ([contrib/gitian-build.py](https://github.com/LambyDev/Lamby/blob/master/contrib/gitian-build.py)). Since this is a completely fresh environment, we haven't yet cloned the lamby repository and will need to fetch this script with the following commands:

```bash
curl -L -O https://raw.githubusercontent.com/LambyDev/Lamby/master/contrib/gitian-build.py
...
- Clone the gitian-builder, gitian.sigs, lamby-detached-sigs, and lamby GitHub repos.
