...
*Setup instructions for a gitian build of lamby using a VM or physical system.*

Gitian is the deterministic build process that is used to build the lamby
...
to the lamby GitHub release page.
...
- [Building lamby](#building-lamby-core)
...
```bash
gpg --output ${VERSION}-linux/${NAME}/lamby-linux-${VERSION%\.*}-build.assert.sig --detach-sign ${VERSION}-linux/$NAME/lamby-linux-${VERSION%\.*}-build.assert
gpg --output ${VERSION}-osx-unsigned/$NAME/lamby-osx-${VERSION%\.*}-build.assert.sig --detach-sign ${VERSION}-osx-unsigned/$NAME/lamby-osx-${VERSION%\.*}-build.assert
gpg --output ${VERSION}-win-unsigned/$NAME/lamby-win-${VERSION%\.*}-build.assert.sig --detach-sign ${VERSION}-win-unsigned/$NAME/lamby-win-${VERSION%\.*}-build.assert
```

Uploading Signatures
--------------------
Make a Pull Request (both the `.assert` and `.assert.sig` files) to the
[gitian.sigs](https://github.com/lamby-project/gitian.sigs/) repository:
...
You can also mail the files to Fuzzbawls (fuzzbawls@lamby-crypto.com) and he will commit them.

```bash
gpg --detach-sign ${VERSION}-linux/${NAME}/lamby-linux-*-build.assert
gpg --detach-sign ${VERSION}-win-unsigned/${NAME}/lamby-win-*-build.assert
gpg --detach-sign ${VERSION}-osx-unsigned/${NAME}/lamby-osx-*-build.assert
```
